function cqu_unified_retrieval_sys_temp1() {
    var unified_retrieval_sys_temp2_temmplate = `
<div v-if="request_of"><div class="temp-loading"></div><!--加载中--></div>
<div id="Search" :style="{background: 'url('+imgPath+'cqu/search-bg.jpg) no-repeat'}" v-else>
    ...
    <div class="content">
      <template v-if="
        ...
        <div class="content-top">
          <div class="content-top-list nav" v-for="(item, index) in onKeywordInputSuggestModel.matchComponent" :key="index">
            <div>
            </div>
          ...
          <div class="block-content-middle-item" :key="index">
            ...
            <div class="block-sel3" v-else>
              <select name="cars" v-model="item.concatExpression">
                <option v-for="expression in supportExpressions2" :key="expression.key" :value="expression.value">
...
<div v-if="block_s" class="bolck-box">
  <div class="ex-body">
    ...
    class searchOptions2 {
        /**支持的逻辑条件 */
        get supportExpressions2() {
            return supportExpressions2;
        }
        /**表示哪些将被限制在rule里面的条件 */
        get limitOnRule() {
    ...
    */
    class searchExpressionCore2 {
        constructor() {
            this._addSimpleSearchConditions = [];
            this._inRulesSearchType = searchOption2.limitOnRule;
        ...
        resolveExpressionForDisplayStirngImpl(expression) {
            ...
            if (IsNullOrWhiteSpace(expression))
                return result;
            for (let index = 0; index < supportExpressions2.length; index++) {
                let element = supportExpressions2[index];
                result = this.replaceAll(result, element.value, element.displayTitle);
            }
            let arr = searchOption2.supportSearchTypes.sort((a, b) => b.key.length - a.key.length);
        ...
        mapHighLevelExpression(displayExpression) {
            ...
            for (let index = 0; index < supportExpressions2.length; index++) {
                const element = supportExpressions2[index];
        ...
        findMinIndexOf(input, position) {
            let temp = supportExpressions2.map(x => {
    ...
    /**检索的一些配置项 */
    const searchOption2 = new searchOptions2();
    /**检索词匹配模式 */
    var searchMatchType;
    ...
    var list = document.getElementsByClassName('cqu_unified_retrieval_sys_temp1');
    for (var i = 0; i < list.length; i++) {
        if (list[i].getAttribute('class').indexOf('jl_vip_zt_vray') < 0) {
            list[i].setAttribute('class', 'cqu_unified_retrieval_sys_temp1 jl_vip_zt_vray jl_vip_zt_warp');
            var unified_retrieval_sys_temp2_set_list = null;
            if (list[i].dataset && list[i].dataset.set) {
                unified_retrieval_sys_temp2_set_list = JSON.parse(list[i].dataset.set.replace(/'/g, '"'));
            }
            new Vue({
                el: '#' + list[i].lastChild.id,
                template: unified_retrieval_sys_temp2_temmplate,

                data() {
                    return {
                        request_of:true,//请求中
                        /**对应 AND OR这些鬼东西 */
                        supportExpressions2: searchOption2.supportExpressions2,
                        ...
                        articleTypes: searchOption2.articleTypes
                            .filter((x) => x.key != 2)
                            .map((x) => {
                                return {
                                    searchType: "TY",
                        ...
                        //主检索处理器
                        searchExpressionResolver: new searchExpressionCore2(),
                        /**检索条件为空的时候出来的提示面板 */
                        ...
                        articleTypes_s: searchOption2.articleTypes.map((x) => {
                            return {
                                searchType: "TY",
                                value: x.key,
                                matchType: searchMatchType.Accurate,
                        ...
                        block: false,
                        block_s: false,
                        baseUrl: window.apiDomainAndPort + "/articlesearch",
                ...
                mounted() {
                    ...
                    if (unified_retrieval_sys_temp2_set_list) {
                        for (var i = 0; i < unified_retrieval_sys_temp2_set_list.length; i++) {
                            var columnid = unified_retrieval_sys_temp2_set_list[i].id;
                            this.initComponentAsync(columnid).then(() => {
                ...
                computed: {
                    ...
                    supportFieds: function () {
                        return searchOption2.limitOnRule

                            .map((x) => {
                                let temp = searchOption2.supportSearchTypes.find((y) => y.key == x);
                ...
                methods: {
                    fiedChange(item) {
                        ...
                        if (item.supportFied == 'A' || searchOption2.accurateOnly.find(x => x == item.supportFied) != null) item.searchMatchType = searchMatchType.Accurate;
                        else item.searchMatchType = searchMatchType.Fuzzy;
                    },
                    /**不通的字段匹配不同的范围条件 */
                    mapSearchMatchTypes(searchType) {

                        if (searchOption2.accurateOnly.find(x => x == searchType) != null)
                    ...
                    /**初始化页面的一些数据 */
                    initComponentAsync(columnid) {
                        return Promise.all([
                            this.getJsonAsync("/api/search-box-configure/search-configure-by-id?id=" + columnid).then((x) => {
                    ...
                    /**进入到检索结果页面 */
                    goToSearch() {
                        let searchConditon = this.searchExpressionResolver.buildApiRules();
                        this.postJsonAsync('/api/search-const/nav-old',searchConditon)
                        .then(x=>{
                    ...
                    rxAutoComplete(x) {
                        ...
                        Promise.all([
                            Promise.resolve(
                                searchOption2.autoMapRegexInfo.find((y) => y.regex.test(x))
                            ),
...
setTimeout(() => { cqu_unified_retrieval_sys_temp1() }, 500)