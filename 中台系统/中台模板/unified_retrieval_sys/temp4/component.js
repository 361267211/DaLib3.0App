function unified_retrieval_sys_temp4() {
    var unified_retrieval_sys_temp4_temmplate = `
<div id="unified_retrieval_sys_temp4_Search" v-if="searchBoxConfigure" :style="{background: 'url('+fileUrl+'/public/image/top-bg.png) no-repeat center'}">
...
<div v-if="block_s" class="bolck-box">
  <div class="ex-body">
    ...
    */
    ...
    var list = document.getElementsByClassName('unified_retrieval_sys_temp4');
    for (var i = 0; i < list.length; i++) {
        if (list[i].getAttribute('class').indexOf('jl_vip_zt_vray') < 0) {
            list[i].setAttribute('class', 'unified_retrieval_sys_temp4 jl_vip_zt_vray jl_vip_zt_warp');
            var unified_retrieval_sys_temp4_set_list = null;
            if (list[i].dataset && list[i].dataset.set) {
                unified_retrieval_sys_temp4_set_list = JSON.parse(list[i].dataset.set.replace(/'/g, '"'));
            }
            new Vue({
                el: '#' + list[i].lastChild.id,
                template: unified_retrieval_sys_temp4_temmplate,
                ...
                mounted() {
                    ...
                    if (unified_retrieval_sys_temp4_set_list) {
                        for (var i = 0; i < unified_retrieval_sys_temp4_set_list.length; i++) {
                            var columnid = unified_retrieval_sys_temp4_set_list[i].id;
...
setTimeout(() => { unified_retrieval_sys_temp4() }, 500)