...
namespace kiwiho.Course.MultipleTenancy.EFcore.Api.Infrastructure
{
    ...
    public static class MultipleTenancyExtension
    {
        ...
        /// <returns></returns>
        public static IServiceCollection AddDatabasePerConnection<TDbContext>(this IServiceCollection services, ConnectionResolverOption option) where TDbContext : DbContext, ITenantDbContext
        {
            if (option == null)
            {
        ...
        /// <param name="option"></param>
        /// <returns></returns>
        internal static IServiceCollection AddDatabase<TDbContext>(this IServiceCollection services, ConnectionResolverOption option) where TDbContext : DbContext, ITenantDbContext
        {
            ...
            services.AddDatabaseAccessor(options =>
            {
                ...
                services.AddDbContext<TDbContext>((serviceProvider, options) =>
                {
                    ...
                    switch (option.DBType)
                    {
                        case DatabaseIntegration.SqlServer:
                            ...
                            dbOptionBuilder = options.UseSqlServer(option.ConnectinString,
                                optionBuilder =>
                                {
                                    if (option.Type == ConnectionResolverType.ByTable)
                                    {
                                        optionBuilder.MigrationsHistoryTable($"{tenant.Name}__EFMigrationsHistory");
                                    }
                                    if (option.Type == ConnectionResolverType.BySchema)
                                    {
                                        optionBuilder.MigrationsHistoryTable("__EFMigrationsHistory", $"{tenant.Name ?? "public"}");
                                    }
                        ...
                        case DatabaseIntegration.PostgreSql:
                            //TODO:连接字符串待配置化
                            dbOptionBuilder = options.UseNpgsql(option.ConnectinString, optionBuilder =>
                                 {
                                     if (option.Type == ConnectionResolverType.ByTable)
                                     {
                                         optionBuilder.MigrationsHistoryTable($"{tenant.Name}__EFMigrationsHistory");
                    ...
                    if (option.Type == ConnectionResolverType.ByTable || option.Type == ConnectionResolverType.BySchema)
                    {
        ...
        /// <param name="option"></param>
        /// <returns></returns>
        public static IServiceCollection AddTenantDatabasePerTable<TDbContext>(this IServiceCollection services, ConnectionResolverOption option) where TDbContext : DbContext, ITenantDbContext
        {
            if (option == null)
            {
                option = new ConnectionResolverOption()
                {
                    Key = "default",
                    Type = ConnectionResolverType.ByTable,
        ...
        /// <param name="option"></param>
        /// <returns></returns>
        public static IServiceCollection AddTenantDatabasePerSchema<TDbContext>(this IServiceCollection services, ConnectionResolverOption option) where TDbContext : DbContext, ITenantDbContext
        {
            if (option == null)
            {
                option = new ConnectionResolverOption()
