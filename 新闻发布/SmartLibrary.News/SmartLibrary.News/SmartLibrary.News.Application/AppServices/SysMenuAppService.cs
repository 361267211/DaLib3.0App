...
namespace SmartLibrary.News.Application.AppServices
{
   ...
   // [DatabaseActionFilter]
    public class SysMenuAppService : IDynamicApiController
    {
        ...
        [UnitOfWork]
        public async Task<PermissionNode> GetUserPermissionTree(string userKey)
        {
        ...
        /// <returns></returns>
        [HttpGet]
        //[AuthorizeMultiplePolicy("Premission1", false)]
        [UnitOfWork]
        public async Task<List<PermissionMenu>> GetUserUnionColumnPermissionList()
        {
            var permissionNode = await _sysMenuService.GetUserUnionColumnPermissionTree();
            return permissionNode;
        }
