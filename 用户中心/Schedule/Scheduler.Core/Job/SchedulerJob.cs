        ...
        public Task Execute(IJobExecutionContext context)
        {
            try
            {
                ...
                foreach (var item in updateJobList)
                {
                    ...
                    {
                        Console.WriteLine("修改任务成功,任务Id:" + item.Id);
                        LogManager.Info(string.Format("修改任务:{0}", JsonHelper.SerializeObject(item)));
                    }
                    else
                    {
                        Console.WriteLine("修改任务失败,Id:" + item.Id);
                    }
                }
                ...
                foreach (var item in addJobList)
                {
                    ...
                    {
                        Console.WriteLine("创建任务成功，任务ID:" + item.Id);
                        LogManager.Info(string.Format("新增任务:{0}", JsonHelper.SerializeObject(item)));
                    }
                    else
                    {
                        Console.WriteLine("创建任务失败，任务ID:" + item.Id);
                    }
                }
                ...
                foreach (var item in deleteJobList)
                {
                    ...
                    {
                        Console.WriteLine("删除任务成功，任务ID:" + item.Id);
                        LogManager.Info(string.Format("删除任务:{0}", JsonHelper.SerializeObject(item)));
                    }
                    else
                    {
                        Console.WriteLine("删除任务失败，任务ID:" + item.Id);
                    }
                }
            }
            catch (Exception ex)
            ...
            Console.WriteLine("运行任务ID:" + string.Join(';', SchedulerCenter.Instance.SchedulerEntityList.Where(c => c.JobStatus == JobStatus.RUN).Select(c => c.Id).ToList()));
            Console.WriteLine("暂停任务ID:" + string.Join(';', SchedulerCenter.Instance.SchedulerEntityList.Where(c => c.JobStatus == JobStatus.PAUSE).Select(c => c.Id).ToList()));
