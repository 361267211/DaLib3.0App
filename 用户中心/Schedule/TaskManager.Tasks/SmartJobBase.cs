...
namespace TaskManager.Tasks
{
    ...
    public abstract class SmartJobBase : JobBase
    {
        ...
        {
            if (SchedulerEntity != null && !string.IsNullOrEmpty(SchedulerEntity.AdapterAssemblyFullName)
                                        && !string.IsNullOrEmpty(SchedulerEntity.AdapterClassFullName))
            {
                try
                {
                ...
                {
                    LogManager.Error("适配器初始化异常,可能是配置适配器路径不正确" + ex, JobName);
